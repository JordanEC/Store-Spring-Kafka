...
public class GetItemsSteps {
    ...
    ResponseEntity<List<Item>> responseEntityList;
    ...
    public static ParameterizedTypeReference<List<Item>> listItemTypeReference()
    {
        return new ParameterizedTypeReference<List<Item>>(){};
    }
    public static ParameterizedTypeReference<Item> itemTypeReference()
    {
        return new ParameterizedTypeReference<Item>(){};
    }
    ...
    public void theGivenItemIsCreated(Item item){
        this.inputItem = item;
        ResponseEntity<Item> responseEntity =
            testRestTemplate
                .postForEntity(buildURL() + "item", item, Item.class);
    ...
    public void theItemIsGetById() {
        ...
        responseEntity = testRestTemplate.getForEntity(buildURL() + "item/{itemId}", Item.class, params);
    }

    @When("The item name description and unit price are updated")
    public void theItemNameDescriptionAndUnitPriceAreUpdated(Item item) {
        this.inputItem = item;
        Map<String, Long> params = new HashMap<>();
        params.put("itemId", itemId);
        HttpEntity<Item> httpEntity = new HttpEntity<>(item);
        responseEntity =
                testRestTemplate.exchange(buildURL() + "item/{itemId}", HttpMethod.PUT, httpEntity, Item.class, params);
        Assert.assertEquals(HttpStatus.OK, responseEntity.getStatusCode());
        itemId = responseEntity.getBody().getItemId();
    }
    ...
    public void theItemIsFetched() {
        ...
        Assert.assertEquals(inputItem.getName(), responseItem.getName());
        Assert.assertEquals(inputItem.getDescription(), responseItem.getDescription());
        Assert.assertEquals(inputItem.getUnitPrice(), responseItem.getUnitPrice());
    }

    @Given("The given items are created")
    public void theGivenItemsAreCreated(List<Item> items) {
        HttpEntity<List<Item>> httpEntity = new HttpEntity<>(items);
        ...
        responseEntityList.getBody().forEach(it -> itemIds.add(((Item)it).getItemId()));
    }
